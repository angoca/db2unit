...
SET CURRENT SCHEMA DB2UNIT_XML @

SET PATH = "SYSIBM","SYSFUN","SYSPROC","SYSIBMADM", DB2UNIT_2_BETA, DB2UNIT_XML @
...
BEGIN
 ...
 SET STATEMENT = 'CREATE SCHEMA DB2UNIT_XML';
...
_END
  ...
  INSERT INTO DB2UNIT_XML.REPORT_TESTS (DATE, EXECUTION_ID,
...
_END
  ...
  SELECT MESSAGE INTO ACTUAL_MSG
    FROM DB2UNIT_XML.REPORT_TESTS
    WHERE DATE = (SELECT MAX(DATE)
      FROM DB2UNIT_XML.REPORT_TESTS);

  DELETE FROM DB2UNIT_XML.REPORT_TESTS
    WHERE EXECUTION_ID = 0
    AND MESSAGE = EXPECTED_MSG
    AND DATE = (SELECT MAX(DATE) FROM DB2UNIT_XML.REPORT_TESTS);
...
_IF __SQLSTATE_429BB _THEN
  SET DOC1 = '<a>Hello World</a>';
  SET DOC2 = '<a>Ciao Mondo</a>';
_ELSE
  SET DOC1 = XMLPARSE(DOCUMENT '<a>Hello World</a>');
  SET DOC2 = XMLPARSE(DOCUMENT '<a>Ciao Mondo</a>');
_END
  ...
  DELETE FROM DB2UNIT_XML.REPORT_TESTS
    WHERE MESSAGE = 'Actual  : "' || DOC2 || '"'
    AND DATE = (SELECT MAX(DATE) FROM DB2UNIT_XML.REPORT_TESTS);
  DELETE FROM DB2UNIT_XML.REPORT_TESTS
    WHERE MESSAGE = 'Expected: "' || DOC1 || '"'
    AND DATE = (SELECT MAX(DATE) FROM DB2UNIT_XML.REPORT_TESTS);
...
_END
  ...
  SELECT MESSAGE INTO ACTUAL_MSG
    FROM DB2UNIT_XML.REPORT_TESTS
    WHERE DATE = (SELECT MAX(DATE) FROM DB2UNIT_XML.REPORT_TESTS);
