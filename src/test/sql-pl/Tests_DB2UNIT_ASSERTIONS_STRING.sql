...
ALTER TABLE DB2UNIT_ASSERTIONS_STRINGS.REPORT_TESTS
  ALTER COLUMN SUITE_NAME
  SET WITH DEFAULT 'DB2UNIT_ASSERTIONS_STRINGS' @
...
CREATE OR REPLACE PROCEDURE TEST_STRING_01()
 BEGIN
  ...
  INSERT INTO DB2UNIT_ASSERTIONS_STRINGS.REPORT_TESTS (DATE, EXECUTION_ID,
  ...
  SELECT MESSAGE INTO ACTUAL_MSG
    FROM DB2UNIT_ASSERTIONS_STRINGS.REPORT_TESTS
    WHERE DATE = (SELECT MAX(DATE) FROM DB2UNIT_ASSERTIONS_STRINGS.REPORT_TESTS);

  DELETE FROM DB2UNIT_ASSERTIONS_STRINGS.REPORT_TESTS
    WHERE EXECUTION_ID = 0
    AND MESSAGE = EXPECTED_MSG
    AND DATE = (SELECT MAX(DATE) FROM DB2UNIT_ASSERTIONS_STRINGS.REPORT_TESTS);
