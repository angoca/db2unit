...
SET PATH = "SYSIBM","SYSFUN","SYSPROC","SYSIBMADM", DB2UNIT_1, DB2UNIT_EXECUTION_2 @
...
--  DELETE FROM DB2UNIT_1.RESULT_TESTS;
 END @
...
CREATE OR REPLACE PROCEDURE TEST_01()
 BEGIN
  ...
  DECLARE ACTUAL INT;
  DECLARE CURR_ID ANCHOR DB2UNIT_1.EXECUTION_REPORTS.EXECUTION_ID;
  ...
  -- Total
  SET EXPECTED = 1;
  SET ACTUAL = (SELECT TOTAL_TESTS FROM DB2UNIT_1.SUITES_EXECUTIONS
  ...
  -- Passed
  SET EXPECTED = 1;
  SET ACTUAL = (SELECT PASSED_TESTS FROM DB2UNIT_1.SUITES_EXECUTIONS
    WHERE SUITE_NAME = SCHEMA_NAME
    AND EXECUTION_ID = CURR_ID
    WITH UR);
  ...
  -- Failed
  SET EXPECTED = 0;
  SET ACTUAL = (SELECT FAILED_TESTS FROM DB2UNIT_1.SUITES_EXECUTIONS
    WHERE SUITE_NAME = SCHEMA_NAME
    AND EXECUTION_ID = CURR_ID
    WITH UR);
  ...
  -- Error
  SET EXPECTED = 0;
  SET ACTUAL = (SELECT ERROR_TESTS FROM DB2UNIT_1.SUITES_EXECUTIONS
    WHERE SUITE_NAME = SCHEMA_NAME
    AND EXECUTION_ID = CURR_ID
    WITH UR);
...
CREATE OR REPLACE PROCEDURE TEST_05_RESULTS()
 BEGIN
  ...
  -- Check values
  SET EXPECTED = 'Passed    ';
  SET ACTUAL = (SELECT FINAL_STATE FROM DB2UNIT_1.RESULT_TESTS
    WHERE SUITE_NAME = SCHEMA_NAME
    AND EXECUTION_ID = CURR_ID
...
CREATE OR REPLACE PROCEDURE TEST_06()
 BEGIN
  ...
  -- Unxecuted tests
  SET EXPECTED = 1;
  SET ACTUAL = (SELECT UNEXEC_TESTS FROM DB2UNIT_1.SUITES_EXECUTIONS
    WHERE SUITE_NAME = SCHEMA_NAME
    AND EXECUTION_ID = CURR_ID
    WITH UR);
