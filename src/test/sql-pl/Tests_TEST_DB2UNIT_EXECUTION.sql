...
CREATE OR REPLACE PROCEDURE TEST_1()
 BEGIN
  ...
  DELETE FROM DB2UNIT_1A.EXECUTION_REPORTS
    WHERE DATE = (SELECT MAX(DATE) FROM DB2UNIT_1A.EXECUTION_REPORTS)
    AND MESSAGE_REPORT = EXPECTED_MSG;

  CALL DB2UNIT_1A.DB2UNIT.ASSERT_STRING_EQUALS(EXPECTED_MSG, ACTUAL_MSG);
 END@
...
CREATE OR REPLACE PROCEDURE TEST_7()
 BEGIN
  ...
  SET EXPECTED_MSG = 'String too long: "SQL0433N  Value "1234567890123456789012'
    || '34567890123456789012345678901234567890" is too long.  SQLSTATE=22001   '
    || '"';
