...
CALL SYSPROC.ADMIN_DROP_SCHEMA('DB2UNIT_TAP_TEST_7', NULL,
  'ERRORSCHEMA', 'ERRORTABLE_7') @
CALL SYSPROC.ADMIN_DROP_SCHEMA('DB2UNIT_TAP_TEST_8', NULL,
...
CREATE OR REPLACE PROCEDURE TEST_1_NO_TEST()
 BEGIN
...
CREATE OR REPLACE PROCEDURE TEST_2_ONE_PASSED()
 BEGIN
...
CREATE OR REPLACE PROCEDURE TEST_3_ONE_FAILED()
 BEGIN
  DECLARE LOGGER_ID SMALLINT;
  DECLARE EXPECTED SMALLINT;
  DECLARE ACTUAL SMALLINT;
...
CREATE OR REPLACE PROCEDURE TEST_4_ONE_ERROR()
 BEGIN
  DECLARE LOGGER_ID SMALLINT;
  DECLARE EXPECTED SMALLINT;
  DECLARE ACTUAL SMALLINT;
...
CREATE OR REPLACE PROCEDURE TEST_5_ALL_TYPES()
 BEGIN
  DECLARE LOGGER_ID SMALLINT;
  DECLARE EXPECTED SMALLINT;
  DECLARE ACTUAL SMALLINT;
  ...
  SET EXPECTED = 14;
  SET ACTUAL = (SELECT COUNT(1) FROM DB2UNIT_2_BETA.TAP_REPORT);
  CALL DB2UNIT.ASSERT_INT_EQUALS('Number', EXPECTED, ACTUAL);

  SET EXPECTED_MSG = 'TAP version 13';
  SET ACTUAL_MSG = (SELECT MESSAGE FROM DB2UNIT_2_BETA.TAP_REPORT
    WHERE NUMBER = 1);
  CALL DB2UNIT_2_BETA.DB2UNIT.ASSERT_STRING_EQUALS('Version', EXPECTED_MSG,
    ACTUAL_MSG);

  SET EXPECTED_MSG = '1..3';
  SET ACTUAL_MSG = (SELECT MESSAGE FROM DB2UNIT_2_BETA.TAP_REPORT
    WHERE NUMBER = 2);
  CALL DB2UNIT_2_BETA.DB2UNIT.ASSERT_STRING_EQUALS('Plan', EXPECTED_MSG,
    ACTUAL_MSG);

  SET EXPECTED_MSG = 'ok 1 TEST_51';
  SET ACTUAL_MSG = (SELECT MESSAGE FROM DB2UNIT_2_BETA.TAP_REPORT
    WHERE NUMBER = 3 FETCH FIRST 1 ROW ONLY);
  CALL DB2UNIT_2_BETA.DB2UNIT.ASSERT_STRING_EQUALS('First', EXPECTED_MSG,
    ACTUAL_MSG);

  SET EXPECTED_MSG = 'not ok 2 TEST_52';
  SET ACTUAL_MSG = (SELECT MESSAGE FROM DB2UNIT_2_BETA.TAP_REPORT
    WHERE NUMBER = 4 FETCH FIRST 1 ROW ONLY);
  CALL DB2UNIT_2_BETA.DB2UNIT.ASSERT_STRING_EQUALS('Second', EXPECTED_MSG,
    ACTUAL_MSG);

  SET EXPECTED_MSG = 'not ok 3 TEST_53';
  SET ACTUAL_MSG = (SELECT MESSAGE FROM DB2UNIT_2_BETA.TAP_REPORT
    WHERE NUMBER = 10 FETCH FIRST 1 ROW ONLY);
  CALL DB2UNIT_2_BETA.DB2UNIT.ASSERT_STRING_EQUALS('Third', EXPECTED_MSG,
    ACTUAL_MSG);
...
CREATE OR REPLACE PROCEDURE TEST_6_EMPTY()
 BEGIN
  DECLARE LOGGER_ID SMALLINT;
  DECLARE EXPECTED SMALLINT;
  DECLARE ACTUAL SMALLINT;
...
-- Tests unstarted test case.
CREATE OR REPLACE PROCEDURE TEST_7_UNSTARTED()
 BEGIN
  DECLARE LOGGER_ID SMALLINT;
  DECLARE EXPECTED SMALLINT;
  DECLARE ACTUAL SMALLINT;
  ...
  CALL LOGGER.GET_LOGGER('DB2UNIT_TAP.TEST_ONE_PASSED.DB2UNIT_TAP_TEST_7',
    LOGGER_ID);

  SET SCHEMA_NAME = 'DB2UNIT_TAP_TEST_7';
  SET TEST_NAME = 'TEST_71';

  SET SENTENCE = 'CREATE OR REPLACE PROCEDURE ' || SCHEMA_NAME || '.SETUP() '
    || 'BEGIN '
    || ' SIGNAL SQLSTATE ''TEST7''; '
    || 'END ';
  CALL LOGGER.ERROR(LOGGER_ID, SENTENCE);
  PREPARE STMT FROM SENTENCE;
  EXECUTE STMT;
  ...
  SET EXPECTED_MSG = 'not ok 1 ' || TEST_NAME || ' # skip';
  SET ACTUAL_MSG = (SELECT MESSAGE FROM DB2UNIT_2_BETA.TAP_REPORT
    WHERE NUMBER = 3 FETCH FIRST 1 ROW ONLY);
  CALL DB2UNIT_2_BETA.DB2UNIT.ASSERT_STRING_EQUALS(EXPECTED_MSG, ACTUAL_MSG);
...
-- Tests unstarted test suite.
CREATE OR REPLACE PROCEDURE TEST_8_UNSTARTED()
 BEGIN
  DECLARE LOGGER_ID SMALLINT;
  DECLARE EXPECTED SMALLINT;
  DECLARE ACTUAL SMALLINT;
  ...
  CALL LOGGER.GET_LOGGER('DB2UNIT_TAP.TEST_ONE_PASSED.DB2UNIT_TAP_TEST_8',
    LOGGER_ID);

  SET SCHEMA_NAME = 'DB2UNIT_TAP_TEST_8';
  SET TEST_NAME = 'TEST_81';
