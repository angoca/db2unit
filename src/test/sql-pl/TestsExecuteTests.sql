...
CREATE OR REPLACE PROCEDURE BEFORE_ALL ()
 BEGIN
...
CREATE OR REPLACE PROCEDURE AFTER_ALL ()
 BEGIN
...
CREATE OR REPLACE PROCEDURE TEST_1 ()
 BEGIN
  ...
  CALL DB2UNIT_TEST.DB2UNIT.EXECUTE_TESTS(SCHEMA_NAME);
  SELECT MESSAGE INTO ACTUAL_MSG
    FROM DB2UNIT_TEST.EXECUTION_REPORTS
    WHERE DATE = (SELECT MAX(DATE) FROM DB2UNIT_TEST.EXECUTION_REPORTS);
