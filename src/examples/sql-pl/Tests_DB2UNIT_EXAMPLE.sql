...
CREATE OR REPLACE PROCEDURE SETUP()
 BEGIN
  DECLARE TEST_NAME VARCHAR(128);
  SET TEST_NAME = DB2UNIT.GET_CURRENT_TESTNAME();
  IF (TEST_NAME = 'TEST_9') THEN
   SIGNAL SQLSTATE 'TEST9' SET MESSAGE_TEXT = 'Normal error test 9';
  END IF;
 END @
...
-- Test an error in setup.
CREATE OR REPLACE PROCEDURE TEST_9()
 BEGIN
  -- Generates an error in setup.
 END @
