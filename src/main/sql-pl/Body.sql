  ...
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
     ...
     INSERT INTO EXECUTION_REPORTS (DATE, EXECUTION_ID, STATUS, MESSAGE_REPORT)
       VALUES (CURRENT TIMESTAMP, EXEC_ID, '', SUBSTR('Exception: SQLCode'
...
ALTER MODULE DB2UNIT publish
 ...
 F_GET_RANDOM_ID : BEGIN
  ...
  CALL LOGGER.GET_LOGGER('DB2UNIT_1B.DB2UNIT.GET_RANDOM_ID', LOGGER_ID);
  CALL LOGGER.WARN(LOGGER_ID, '>');
   ...
   ELSE
    ...
    IF (PROC_NAME_FOUND IS NULL) THEN
     SET PROC_NAME_FOUND = (SELECT PROCNAME
       FROM SYSCAT.PROCEDURES
       WHERE PROCSCHEMA = PROCESS_SCHEMA(CUR_SCHEMA)
       AND PROCNAME = UPPER(PROC_NAME)
       AND LANGUAGE = 'SQL'
       AND PARM_COUNT = 0);
     IF (PROC_NAME_FOUND IS NOT NULL) THEN
      SET PROCS_NAMES[1] = PROC_NAME;
     ELSE
      SET CONTINU = FALSE;
      INSERT INTO EXECUTION_REPORTS (DATE, EXECUTION_ID, STATUS, MESSAGE_REPORT)
        VALUES (CURRENT TIMESTAMP, EXEC_ID, CURRENT_STATUS, 'Test "'
        || PROC_NAME || '" does not exist in suite ' || CUR_SCHEMA);
      CALL LOGGER.ERROR(LOGGER_ID, 'Test "' || PROC_NAME
...
ALTER MODULE DB2UNIT ADD
 ...
 P_RESET_TABLES: BEGIN
  ...
  DELETE FROM DB2UNIT_1B.SORTS;
  DELETE FROM DB2UNIT_1B.SUITES_EXECUTIONS;
  DELETE FROM DB2UNIT_1B.SUITE_LOCKS;
  DELETE FROM DB2UNIT_1B.EXECUTION_REPORTS;
  DELETE FROM DB2UNIT_1B.SUITES;
  DELETE FROM DB2UNIT_1B.EXECUTIONS;
 END P_RESET_TABLES @
...
ALTER MODULE DB2UNIT ADD
 ...
 P_RELEASE_LOCK: BEGIN
  ...
  CALL LOGGER.GET_LOGGER('DB2UNIT_1B.DB2UNIT.RELEASE_LOCK',
    LOGGER_ID);
