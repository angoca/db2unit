...
ALTER MODULE DB2UNIT ADD
 ...
 P_WRITE_TEXT: BEGIN
  ...
  -- TODO borrar? CALL UTL_FILE.NEW_LINE(UTL_FILE_HANDLER);
...
ALTER MODULE DB2UNIT ADD
 ...
 P_EXTRACT_SUITE: BEGIN
  ...
  -- TODO This should be different according to platform
  ...
  END IF;
-- TODO Reduce the utl_file calls
  ...
  CALL UTL_FILE.PUT_LINE(UTL_FILE_HANDLER, '/**');
  CALL UTL_FILE.PUT_LINE(UTL_FILE_HANDLER, ' * TODO Description of the tests suite.');
  CALL UTL_FILE.PUT_LINE(UTL_FILE_HANDLER, ' *');
...
ALTER MODULE DB2UNIT ADD
 ...
 F_GET_HEADER_PROC_FILE_TEST: BEGIN
  ...
  SET RET = RET || ' */\n';
  SET RET = RET || '\n';
  SET RET = RET || 'SET PATH = "SYSIBM","SYSFUN","SYSPROC","SYSIBMADM",' || SCHEMA || ' @\n';
  SET RET = RET || '\n';
...
ALTER MODULE DB2UNIT ADD
 ...
 F_GET_HEADER_PROC_TEST: BEGIN
  ...
  SET RET = '';
  SET RET = RET || '-- TODO Copy this procedure as many times as necessary for the different tests.\n';
  ...
  SET AT_END = FALSE;
  OPEN PARAMS_CURSOR;
  FETCH PARAMS_CURSOR INTO TYPESCHEMA, TYPENAME, PARMNAME, LENGTH, SCALE, ROWTYPE;
  WHILE (AT_END = FALSE) DO
   CASE
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'BOOLEAN' AND ROWTYPE = 'P' THEN
     SET RET = RET || '  DECLARE ' || PARMNAME || ' BOOLEAN;\n';
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'BOOLEAN' AND ROWTYPE IN ('O', 'B') THEN
     SET RET = RET || '  DECLARE EXPECTED_' || PARMNAME || ' BOOLEAN;\n';
     SET RET = RET || '  DECLARE ACTUAL_' || PARMNAME || ' BOOLEAN;\n';
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'TIMESTAMP' AND ROWTYPE = 'P' THEN
     SET RET = RET || '  DECLARE ' || PARMNAME || ' TIMESTAMP;\n';
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'TIMESTAMP' AND ROWTYPE IN ('O', 'B') THEN
    ...
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'DATE' AND ROWTYPE IN ('O', 'B') THEN
    ...
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'TIME' AND ROWTYPE IN ('O', 'B') THEN
    ...
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'DECIMAL' AND ROWTYPE = 'P' THEN
     SET RET = RET || '  DECLARE ' || PARMNAME || ' DECIMAL(' || LENGTH || ',' || SCALE || ');\n';
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'DECIMAL' AND ROWTYPE IN ('O', 'B') THEN
     SET RET = RET || '  DECLARE EXPECTED_' || PARMNAME || ' DECIMAL(' || LENGTH || ',' || SCALE || ');\n';
     SET RET = RET || '  DECLARE ACTUAL_' || PARMNAME || ' DECIMAL(' || LENGTH || ',' || SCALE || ');\n';
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'DOUBLE' AND ROWTYPE = 'P' THEN
     SET RET = RET || '  DECLARE ' || PARMNAME || ' DOUBLE;\n';
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'DOUBLE' AND ROWTYPE IN ('O', 'B') THEN
    ...
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'FLOAT' AND ROWTYPE IN ('O', 'B') THEN
    ...
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'DECFLOAT' AND ROWTYPE IN ('O', 'B') THEN
    ...
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'REAL' AND ROWTYPE IN ('O', 'B') THEN
    ...
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'SMALLINT' AND ROWTYPE IN ('O', 'B') THEN
    ...
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'INTEGER' AND ROWTYPE IN ('O', 'B') THEN
    ...
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'BIGINT' AND ROWTYPE IN ('O', 'B') THEN
    ...
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'CHARACTER' AND ROWTYPE IN ('O', 'B') THEN
     SET RET = RET || '  DECLARE EXPECTED_' || PARMNAME || ' CHARACTER(' || LENGTH || ');\n';
     SET RET = RET || '  DECLARE ACTUAL_' || PARMNAME || ' CHARACTER(' || LENGTH || ');\n';
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'VARCHAR' AND ROWTYPE = 'P' THEN
     SET RET = RET || '  DECLARE ' || PARMNAME || ' VARCHAR(' || LENGTH || ');\n';
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'VARCHAR' AND ROWTYPE IN ('O', 'B') THEN
     SET RET = RET || '  DECLARE EXPECTED_' || PARMNAME || ' VARCHAR(' || LENGTH || ');\n';
  ...
  SET RET = RET || '\n';
  SET RET = RET || '  -- TODO Delete if the procedure does not raise any exception.\n';
  SET RET = RET || '  DECLARE CONTINUE HANDLER FOR SQLSTATE ''99999''\n';
  SET RET = RET || '    BEGIN\n';
  SET RET = RET || '     -- TODO What to do in case of an exception.\n';
  SET RET = RET || '     CALL DB2UNIT.FAIL(''TODO Put a message about why it failed'');\n';
  SET RET = RET || '    END;\n';
  SET RET = RET || '\n';
  SET RET = RET || '  CALL DB2UNIT.REGISTER_MESSAGE(''TODO Put a description.'');\n';
  SET RET = RET || '\n';
   ...
   CASE
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'BOOLEAN' THEN
     ...
     SET RET = RET || '  CALL DB2UNIT.ASSERT_BOOLEAN_NULL (''TODO Message'', '
       || 'ACTUAL_' || PARMNAME || ');\n';
     SET RET = RET || '  CALL DB2UNIT.ASSERT_BOOLEAN_NOT_NULL (''TODO Message'', '
       || 'ACTUAL_' || PARMNAME || ');\n';
    WHEN TYPESCHEMA = 'SYSIBM' AND TYPENAME = 'TIMESTAMP' THEN
     SET RET = RET || '  CALL DB2UNIT.ASSERT_TIMESTAMP_EQUALS (''TODO Message'', '
       || 'EXPECTED_' || PARMNAME || ', ACTUAL_' || PARMNAME || ');\n';
     SET RET = RET || '  CALL DB2UNIT.ASSERT_TIMESTAMP_NULL (''TODO Message'', '
       || 'ACTUAL_' || PARMNAME || ');\n';
     SET RET = RET || '  CALL DB2UNIT.ASSERT_TIMESTAMP_NOT_NULL (''TODO Message'', '
       || 'ACTUAL_' || PARMNAME || ');\n';
    ...
    WHEN TYPESCHEMA = 'SYSIBM' AND (TYPENAME = 'DECIMAL' OR TYPENAME = 'DOUBLE'
     ...
     SET RET = RET || '  CALL DB2UNIT.ASSERT_DEC_NOT_NULL (''TODO Message'', '
       || 'ACTUAL_' || PARMNAME || ');\n';
    WHEN TYPESCHEMA = 'SYSIBM' AND (TYPENAME = 'SMALLINT' OR TYPENAME = 'INTEGER'
      OR TYPENAME = 'BIGINT') THEN
    ...
    WHEN TYPESCHEMA = 'SYSIBM' AND (TYPENAME = 'CHARACTER'
     ...
     SET RET = RET || '  CALL DB2UNIT.ASSERT_STRING_NULL (''TODO Message'', '
       || 'ACTUAL_' || PARMNAME || ');\n';
     SET RET = RET || '  CALL DB2UNIT.ASSERT_STRING_NOT_NULL (''TODO Message'', '
       || 'ACTUAL_' || PARMNAME || ');\n';
    ELSE
     -- Unknown type
     SET RET = RET || '  -- TODO This is an unknown type, and db2unit has not '
