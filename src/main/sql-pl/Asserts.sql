...
ALTER MODULE DB2UNIT ADD
  ...
  LANGUAGE SQL
  SPECIFIC F_CHECK_COLUMN
  MODIFIES SQL DATA
  NOT DETERMINISTIC
  NO EXTERNAL ACTION
  PARAMETER CCSID UNICODE
 F_CHECK_COLUMN: BEGIN
  DECLARE LOGGER_ID SMALLINT;
  ...
  IF (EXPECTED_TOTAL <> ACTUAL_TOTAL) THEN
   CALL WRITE_IN_REPORT (MESSAGE || 'The quantity of different items is not the same in column: '
     || COLNAME);
   SET RET = 13;
  END IF;
  ...
  IF (RET = 0) THEN
   ...
   CALL LOGGER.WARN(LOGGER_ID, 'ANTES');

   WHILE(AT_END = FALSE AND DIFFERENT = FALSE) DO
   CALL LOGGER.WARN(LOGGER_ID, 'OPT1');
    IF (EXPECTED_VALUE <> ACTUAL_VALUE) THEN
     CALL WRITE_IN_REPORT (MESSAGE || 'Different values in column: ' || COLNAME);
     SET DIFFERENT = TRUE;
     SET RET = 11;
   CALL LOGGER.WARN(LOGGER_ID, 'OPT2');
    ELSEIF (EXPECTED_QTY <> ACTUAL_QTY) THEN
     CALL WRITE_IN_REPORT (MESSAGE || 'Different quantity of values in column: '
       || COLNAME);
     SET DIFFERENT = TRUE;
     SET RET = 12;
   CALL LOGGER.WARN(LOGGER_ID, 'OPT3');
    END IF;
    FETCH EXPECTED_CURSOR INTO EXPECTED_VALUE, EXPECTED_QTY;
    FETCH ACTUAL_CURSOR INTO ACTUAL_VALUE, ACTUAL_QTY;
   END WHILE;
  CALL LOGGER.WARN(LOGGER_ID, 'DESPUES');
  END IF;
...
ALTER MODULE DB2UNIT ADD
 ...
 P_ASSERT_TABLE_EQUALS_MESSAGE: BEGIN
  ...
  IF (EXPECTED_SCHEMA IS NULL OR EXPECTED_TABLE_NAME IS NULL
    ...
    CALL WRITE_IN_REPORT (MESSAGE || 'Some of the entry values are null: '
      || COALESCE(EXPECTED_SCHEMA, 'NULL') || ',' 
      || COALESCE(EXPECTED_TABLE_NAME, 'NULL') || ',' 
      || COALESCE(ACTUAL_SCHEMA, 'NULL') || ',' 
      || COALESCE(ACTUAL_TABLE_NAME, 'NULL'));
    SET RET = 14;
  END IF;
  ...
  IF (RET = 0) THEN
   ...
   WHILE (INDEX < EXP_QTY AND RET = 0) DO
    ...
    SET RET = CHECK_COLUMN(MESSAGE, EXPECTED_SCHEMA, EXPECTED_TABLE_NAME,
